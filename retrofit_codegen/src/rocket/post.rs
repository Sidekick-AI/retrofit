...
pub fn post_api(header: TokenStream, function: TokenStream) -> TokenStream {
    ...
    let unpack_args = if args.is_empty() {
        quote! {}
    } else {
        quote! {let #data_struct_ident{ #(#arg_idents),* } = (*data).clone();}
    };
    let (route_header, route_args, pass_through_state) = if args.is_empty() {
        if has_state {
            let state = parse_macro_input!(header as Type);
            (
                quote! {#[rocket::post(#route_path)]},
                quote! {state : &rocket::State<#state>, _secure: #secure_struct_ident},
                quote! {&**state},
            )
        } else {
            (
                quote! {#[rocket::post(#route_path)]},
                quote! {_secure: #secure_struct_ident},
                quote! {},
            )
        }
    } else if has_state {
